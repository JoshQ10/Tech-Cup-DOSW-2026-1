...
@Component
public class TournamentRequestValidator {

    private static final int MIN_TEAMS = 2;

    public void validate(TournamentRequest request) {
        Map<String, String> errors = new HashMap<>();

        if (request.getName() == null || request.getName().isBlank()) {
            errors.put("name", "El nombre del torneo es requerido");
        }
        ...
        if (!errors.isEmpty()) {
            throw new ValidationException("Errores de validacion en la creacion del torneo", errors);
        }
    }
}
