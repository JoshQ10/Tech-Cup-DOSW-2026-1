...
class PlayerServiceImplSearchTest {
    ...
    void testSearchPlayersByPosition() {
        // Arrange
        Page<SportProfile> playersPage = new PageImpl<>(Arrays.asList(player1), pageable, 1);
    ...
    void testSearchAllAvailablePlayers() {
        // Arrange
        Page<SportProfile> playersPage = new PageImpl<>(
                Arrays.asList(player1, player2), pageable, 2);
        when(sportProfileRepository.findAllAvailable(pageable))
                .thenReturn(playersPage);
        when(userRepository.findById(1L)).thenReturn(java.util.Optional.of(user1));
        ...
        verify(sportProfileRepository).findAllAvailable(pageable);
    }
    ...
    void testSearchPlayersEmptyResults() {
        // Arrange
        Page<SportProfile> emptyPage = new PageImpl<>(Arrays.asList(), pageable, 0);
        when(sportProfileRepository.searchAvailablePlayers(
    ...
    void testSearchResponseIncludesPaginationInfo() {
        // Arrange
        Page<SportProfile> playersPage = new PageImpl<>(
                Arrays.asList(player1, player2), pageable, 25);
        when(sportProfileRepository.findAllAvailable(pageable))
                .thenReturn(playersPage);
        when(userRepository.findById(anyLong())).thenReturn(java.util.Optional.of(user1));
        when(sportProfileMapper.toResponse(any())).thenReturn(
